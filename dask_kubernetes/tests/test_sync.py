...
def dont_test_pod_template_yaml(image_name, loop, ns):
    test_yaml = {
        "kind": "Pod",
    ...
    with tmpfile(extension="yaml") as fn:
        ...
        with KubeCluster(f.name, loop=loop, namespace=ns) as cluster:
...
def test_pod_template_yaml_expand_env_vars(image_name, loop, ns):
    try:
...
def test_pod_template_dict(image_name, loop, ns):
    spec = {
        "metadata": {},
        "restartPolicy": "Never",
        "spec": {
    ...
    with KubeCluster(spec, loop=loop, namespace=ns) as cluster:
        cluster.scale(2)
        with Client(cluster, loop=loop) as client:
            future = client.submit(lambda x: x + 1, 10)
            result = future.result()
...
def test_pod_template_minimal_dict(image_name, loop, ns):
    spec = {
        "spec": {
            "containers": [
                {
